...
class TioPopupComponent
  ...
  TioPopupComponent(
      ...
      @Optional() @SkipSelf() TioPopupComponent parentPopup,
  ...
  Future<void> _open() {
    ...
    if (_popupSizeProvider != null) {
      _windowResizeSub = window.onResize
          .transform<Event>(debounceStream(const Duration(milliseconds: 200)))
          .listen((_) {
        _updatePopupMinMaxSize();
      });
    }
  ...
  void _reposition(Object _) {
    ...
    if (state.constrainToViewport) {
      ...
      _repositionOffsetX += viewportShift.left.toInt();
      _repositionOffsetY += viewportShift.top.toInt();
    }
...
class MaterialPopupRef implements PopupRef {
  final TioPopupComponent _popupComponent;
